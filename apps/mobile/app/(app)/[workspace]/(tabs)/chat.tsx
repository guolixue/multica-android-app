...
import { useLastChatSessionStore } from "@/data/stores/last-chat-session-store";
...
export default function ChatTab() {
  ...
  // The persisted "last-active session" for the current workspace, restored
  // from SecureStore before hydration. Resuming the conversation the user
  // was actually last in ("上次聊的对话") beats blindly opening the list's
  // first row — a pinned chat or a freshly-created empty session can sort
  // above it.
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void useLastChatSessionStore.getState().restore(wsId).then((id) => {
      if (cancelled) return;
  ...
  // Persist the active session so a cold start / re-entry resumes the
  // conversation the user was in. No-op when it hasn't changed.
  // Gated on `!sessionsLoading`: right after a workspace switch the local
  // `activeSessionId` still holds the OLD workspace's session, and writing
  // it under the new workspace's key would clobber the value the restore
  // effect is about to read. Once sessions settle, hydration replaces the
  // stale id before the next persist fires.
  useEffect(() => {
    if (!wsId) return;
    if (sessionsLoading) return;
    void useLastChatSessionStore.getState().persist(wsId, activeSessionId);
  }, [wsId, sessionsLoading, activeSessionId]);

  useEffect(() => {
    if (!wsId) return;
    if (hydratedWsRef.current === wsId) return;
    // Wait for the last-session restore to settle before deciding the
    // target. Restore is async (SecureStore), and the sessions fetch must
    // also have landed — the initial render has `sessions === []` while the
    // query is still in flight; marking the workspace hydrated there would
    // lock the tab onto the blank new-chat state and never open the last
    // chat once the data lands (the guard above then blocks the retry).
    if (!lastSessionLoaded) return;
    if (sessionsLoading) return;
    hydratedWsRef.current = wsId;
    if (sessions.length === 0) return;
    const lastId = lastSessionIdRef.current;
    const lastSession = lastId
      ? sessions.find((s) => s.id === lastId)
      : undefined;
    // Prefer the persisted last-active session when it still exists in the
    // list; otherwise fall back to the first row (most recently active per
    // server ordering COALESCE(last_message_at, updated_at) DESC).
    setActiveSessionId(lastSession?.id ?? sessions[0].id);
  }, [wsId, sessions, sessionsLoading, lastSessionLoaded]);
  ...
  // Order the transcript by created_at ASC, id ASC — the same order the
  // server returns (ListChatMessages) and the order web renders. The cache
  // itself is insertion-ordered: optimistic sends and WS `chat:done` both
  // append, so when a follow-up is sent while the previous AI reply is still
  // landing, the append order is `… B, A` and the transcript would otherwise
  // show B above A. Sorting here fixes every insertion path at once.
  //
  // Optimistic entries (id `optimistic-…`) are pinned to the END: they carry
  // a client-clock timestamp which can be skewed relative to the server, so
  // sorting them by created_at could float a just-sent message above an
  // earlier AI reply. A just-sent message is always the newest user action,
  // so the bottom is the correct position until the server confirms it and
  // the entry is replaced with a real server timestamp.
  const isOptimistic = (id: string) => id.startsWith("optimistic-");
  const visibleMessages = useMemo(
    () =>
      hideQueuedChatMessages(messages, pendingTask).sort((a, b) => {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      {/* KeyboardAvoidingView from react-native-keyboard-controller, not
          react-native: the RN one is a no-op on Android (behavior must be
          undefined there), so the composer would stay hidden behind the
          IME. This version lifts by the measured keyboard height on both
          platforms and coordinates with the root KeyboardProvider. */}
