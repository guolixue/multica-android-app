...
/** Pixel slack at the top edge — inside this band we treat the user as
 *  "already at top" so the scroll-to-top arrow hides once the header
 *  (title / description) is fully visible. Mirrors AT_BOTTOM_SLACK_PX. */
const AT_TOP_SLACK_PX = 20;
...
export function TimelineList({
  ...
  //   1. sort by (created_at, id) — the cache can drift out of order: the
  //      optimistic comment insert appends without sorting, and WS insert
  //      timing can place a newer follow-up above an in-flight AI reply.
  //      Sorting here (a copy, never the cache) restores chronological
  //      order for every insertion path, mirroring ListChatMessages.
  //      Optimistic entries (id `optimistic-…`) are pinned to the END —
  //      they carry a client-clock timestamp that can be skewed relative to
  //      the server, so sorting by created_at could float a just-posted
  //      comment above the AI reply it followed. A just-posted comment is
  //      always the newest user action, so the bottom is correct until the
  //      server confirms it and the refetch swaps in the real row.
  //   2. coalesceTimeline → merge consecutive identical activities
  //   3. buildTimelineRows → reorder so replies sit adjacent to their parent
  ...
  const data = useMemo<TimelineRow[]>(() => {
    ...
    const sorted = [...entries].sort((a, b) => {
      const ao = isOptimistic(a.id);
      const bo = isOptimistic(b.id);
      if (ao !== bo) return ao ? 1 : -1;
      return (
        a.created_at.localeCompare(b.created_at) || a.id.localeCompare(b.id)
      );
    });
    return buildTimelineRows(coalesceTimeline(sorted));
  }, [entries]);
  ...
  // Mirrors isAtBottomRef as render state so the "scroll to latest" arrow
  // can hide once the user is already at the bottom.
  const [atBottom, setAtBottom] = useState(true);
  // Mirror of the top-edge scroll state for the "scroll to top" arrow.
  //
  // Initial value is tied to the deep-link: an Inbox open lands the timeline
  // at the BOTTOM (startRenderingFromBottom + remount below), so the up-arrow
  // must be visible from the first paint — FlashList does not guarantee an
  // onScroll on mount to correct the state. A normal open starts at the top,
  // where the arrow stays hidden until the user scrolls down.
  const [atTop, setAtTop] = useState(!highlightCommentId);
  const lastDataLenRef = useRef(0);
  // ── Bottom pull-to-refresh tracking ───────────────────────────────────
  // Mirror of the top RefreshControl: when the user is at the bottom and
  // swipes up (an upward drag that stays at the bottom edge), refresh.
  // RN's RefreshControl only anchors to the top edge, so this is hand-rolled
  // from scroll events. On Android the content is clamped at the bottom and
  // produces NO onScroll during a bottom pull, so we capture the pre-drag
  // offset in onScrollBeginDrag (from the last known scroll position) and
  // compare it against the offset at onScrollEndDrag.
  ...
  const handleScroll = useCallback(
    (e: NativeSyntheticEvent<NativeScrollEvent>) => {
      ...
      const nextAtBottom = distFromBottom < AT_BOTTOM_SLACK_PX;
      isAtBottomRef.current = nextAtBottom;
      setAtBottom(nextAtBottom);
      // Top edge — mirror of the bottom check, drives the scroll-to-top
      // arrow. contentOffset.y is 0 at rest (RefreshControl pulls from here).
      setAtTop(contentOffset.y < AT_TOP_SLACK_PX);
      ...
      if (!wasAtBottom && nextAtBottom && newCount > 0) {
        setNewCount(0);
      }
    },
    [newCount],
  ...
  const handleScrollEndDrag = useCallback(
    (e: NativeSyntheticEvent<NativeScrollEvent>) => {
      ...
      if (!isAtBottomRef.current) return;
      // An upward-finger swipe at the bottom keeps contentOffset at (or past)
      // the bottom edge — final offset >= start. If the user instead scrolled
      // up toward the top, offset dropped below start and this is a normal
      // scroll, not a pull-up-to-refresh.
  ...
  // "Scroll to latest" — the persistent down-arrow on the right. Same
  // scrollToEnd as the new-comment chip, just always available (hidden
  // once the user is already at the bottom).
  ...
  // "Scroll to top" — the up-arrow on the right, complement to the
  // down-arrow. Hidden while the header (title / description) is fully
  // visible. FlashList's scrollToOffset(0) lands on the very first row,
  // showing the whole header + the earliest timeline entry.
  ...
  return (
    ...
    <View className="flex-1">
      ...
      <FlashList
        ...
        // Bottom pull-to-refresh indicator — mirrors the top RefreshControl
        // spinner. Shown while a bottom-initiated refresh runs.
        ListFooterComponent={
          refreshing ? (
      ...
      {!atTop ? (
        <ScrollToTopButton onPress={onScrollToTop} />
      ) : null}
      {!atBottom ? (
        <ScrollToLatestButton onPress={onScrollToLatest} />
      ) : null}
    </View>
...
/**
 * Skeleton for the Activity section — shown only while the timeline query
 * is cold-loading (no restored cache, no in-memory data). Mirrors the
 * comment-row layout (avatar + name + body lines) so the eye sees the
 * list structure immediately instead of a bare spinner, matching the
 * IssueDetailLoading skeleton pattern. When the timeline was restored
 * from the disk-persisted cache, this never renders — the last-known
 * entries show instantly and refresh behind the scenes.
 */
...
/**
 * Floating "scroll to latest" down-arrow, anchored bottom-right of the
 * timeline. Appears whenever the user has scrolled up from the newest
 * entry (long conversations) and disappears once they're back at the
...
function ScrollToLatestButton({ onPress }: { onPress: () => void }) {
  ...
  return (
    ...
    >
      <Ionicons name="arrow-down" size={20} color={fg} />
    </Pressable>
  );
}

/**
 * Floating "scroll to top" up-arrow, anchored bottom-right stacked above the
 * "scroll to latest" down-arrow — the familiar jump-FAB corner. Appears once
 * the user has scrolled down from the top (or lands at the bottom via an
 * Inbox deep-link), and disappears when they're back at the very top.
...
function ScrollToTopButton({ onPress }: { onPress: () => void }) {
  ...
  return (
    ...
    >
      <Ionicons name="arrow-up" size={20} color={fg} />
    </Pressable>
  );
}
