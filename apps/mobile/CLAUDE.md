...
### Android APK builds MUST bump the version

Every APK build bumps the patch version by 1 (`0.2.0 → 0.2.1`) so a new APK
installs as an update over the previous one. `versionCode` is derived from the
semver (`major*10000 + minor*100 + patch`, so `0.2.1 → 20001`) — it increases
monotonically with the patch bump and Android treats it as an upgrade.

**Never run `gradlew assembleRelease` directly.** Use the wrapper script:

- `pnpm build:android:apk` — bump version, build arm64-v8a release APK, then
  copy it to a versioned filename (repo root or `apps/mobile`).
- `pnpm build:android:apk:universal` — same, but builds for all ABIs.

The bump writes the new version into all four places that carry it:
`package.json` (root + `apps/mobile/`), `app.config.ts`
(`version` + `android.versionCode`), and the generated
`android/app/build.gradle` (`versionName` + `versionCode`). `android/` is
generated by `expo prebuild` and is git-ignored — on a fresh checkout run
`pnpm -C apps/mobile exec expo prebuild --platform android` once before the
first APK build so the script has a `build.gradle` to update.

**The APK filename carries the version.** Gradle always emits
`android/app/build/outputs/apk/release/app-release.apk`; the wrapper's final
step copies it to `apps/mobile/dist/multica-app-v<version>.apk` (e.g.
