 ...
 *
 * `staleTime: Infinity` on live caches — kept fresh by WS event handlers,
 * not by background refetch. Foreground / reconnect invalidates are scoped
 * to each owning hook (see use-chat-sessions-realtime.ts and
 * use-chat-session-realtime.ts). Message transcripts additionally persist
 * to AsyncStorage (query-client.ts) and are invalidated on restore so a
 * cold start renders the last-known conversation instantly, then refetches
 * behind the scenes (placeholderData keeps it on screen during the refetch).
 */
...
export const chatSessionsOptions = (wsId: string | null) =>
  queryOptions({
    ...
    enabled: !!wsId,
    staleTime: Infinity,
    // Keep the previous list while refetching (see inboxListOptions).
