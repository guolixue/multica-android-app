...
class ApiClient {
  ...
  private async fetch<T>(
    ...
    if (!res.ok) {
      ...
      // Server error bodies are `{ "error": "<msg>", "code"? }` (writeError /
      // writeErrorCode). Read `error` FIRST — the previous code only looked
      // at `message`, so the server's actual reason (e.g. "please wait before
      // requesting another code") was dropped and users only ever saw
      // "500 Internal Server Error". Prefer `error`, then `message`, then a
      // status fallback.
