...
import { commentToTimelineEntry } from "@/data/realtime/issue-ws-updaters";
...
export function useCreateComment(issueId: string) {
  ...
  return useMutation({
    ...
    // Swap the optimistic entry for the server-issued comment the moment the
    // POST resolves, rather than waiting for the invalidate refetch below.
    // Without this, the optimistic entry (`optimistic-…`) and the WS
    // `comment:created` broadcast's real entry (UUID) coexist until the
    // refetch lands — the user briefly sees the same comment twice. Replacing
    // here closes that window: even if the WS event fires afterward,
    // appendTimelineEntry's id-dedup sees the real id already present and
    // skips. The invalidate stays as a consistency net (server ordering,
    // entries missed while the POST was in flight) — it no longer drives
    // duplicate-free rendering.
    onSuccess: (data, vars, ctx) => {
      if (ctx?.optimisticId) {
        const realEntry = commentToTimelineEntry(data);
        qc.setQueryData<TimelineEntry[]>(ctx.key, (old) => {
          if (!old) return old;
          const withoutOptimistic = old.filter(
            (e) => e.id !== ctx.optimisticId,
          );
          // WS may have already appended the real entry before the POST
          // resolved — don't add a second copy.
          if (withoutOptimistic.some((e) => e.id === realEntry.id)) {
            return withoutOptimistic;
          }
          const next = [...withoutOptimistic, realEntry];
          next.sort((a, b) => {
